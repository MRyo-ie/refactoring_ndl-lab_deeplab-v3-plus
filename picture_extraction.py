...
from data_in.utils import dataset_util
...
def make_xml(filepath, width, height, cordinates_lst):
    ...
    # print(tree)
    tree.write(os.path.join(FLAGS.output_dir, filename) +
               ".xml", xml_declaration=False)
...
def main(unused_argv):
    ...
    for pred_dict, image_path in zip(predictions, image_files):
        ...
        for index, c in enumerate(contours):
            ...
            figure_cropped_margin = img[ymar_min:ymar_max,
                                        xmar_min:xmar_max].copy()
            fh, fw = figure_cropped_margin.shape[:2]
            cord_lst.append(
                ["graphic", xmar_min, ymar_min, xmar_max, ymar_max])
            figure_cropped_margin = cv2.resize(
                figure_cropped_margin, (fw*width_r//1600, fh*height_r//1600))
            cv2.imwrite(os.path.join(output_dir, os.path.basename(
                image_path)+"_"+str(index)+".jpg"), figure_cropped_margin)
